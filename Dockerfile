FROM node:16-slim

# Update apt packages
RUN runDeps="openssl ca-certificates patch gosu git make tmux locales-all curl" \
  && apt-get update \
  && apt-get install -y --no-install-recommends $runDeps \
  && apt-get clean \
  && rm -rf /var/lib/apt/lists/* \
  && npm install -g mrs-developer \
  && mkdir -p /opt/frontend/src/addons \
  && rm -rf /opt/frontend/src/addons/* \
  && find /opt/frontend/ -not -user node -exec chown node {} \+ \
  && corepack enable

COPY . /opt/frontend/
COPY entrypoint.sh /opt/frontend/entrypoint.sh

RUN rm -rf /opt/frontend/src/addons/*
RUN chown -R root /opt/frontend
RUN cp /opt/frontend/jsconfig.json.prod /opt/frontend/jsconfig.json

USER root

WORKDIR /opt/frontend/

RUN cd /opt/frontend \
  && make develop \
  && corepack enable \
  && yarn \
  && yarn build

# RAZZLE_API_PATH=VOLTO_API_PATH RAZZLE_INTERNAL_API_PATH=VOLTO_INTERNAL_API_PATH
# RUN rm -rf /home/node/.cache

EXPOSE 3000 3001

HEALTHCHECK --interval=1m --timeout=3s \
  CMD curl -f http://localhost:3000/ || exit 1

# ENTRYPOINT ["/opt/frontend/entrypoint.sh"]

CMD ["yarn", "start:prod"]

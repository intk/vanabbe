...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t intk/volto-vanabbemuseum-theme-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t intk/volto-vanabbemuseum-theme-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name volto-vanabbemuseum-theme-backend-acceptance -d intk/volto-vanabbemuseum-theme-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name volto-vanabbemuseum-theme-frontend-acceptance --link volto-vanabbemuseum-theme-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d intk/volto-vanabbemuseum-theme-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop volto-vanabbemuseum-theme-frontend-acceptance
	@docker stop volto-vanabbemuseum-theme-backend-acceptance

...
class AdminFixes(BrowserView):
    ...
    def reindex_decades(self):
        ...
        for count, brain in enumerate(brains):
            brain.getObject().reindexObject(idxs=["decades"], update_metadata=True)
            if count % 1000 == 0:
                transaction.commit()
            ...
            for lang in intl.keys():
                ...
                if len(fields) > 1:
                    ...
                    for el in fields:
                        ...
                        scope = el.get("Scope")
                        if title or scope:
                            info[lang]["objectDescription_extra"] = str(el.text)
    ...
    def fix_untitled(self):
        ...
        ens = catalog(
            portal_type=["publication", "artwork", "exhibition"], Language="en"
        )
        ...
        for b in ens:
            if not b.Title:
                ...
                obj = b.getObject()
                trans = catalog(TranslationGroup=b.TranslationGroup, Language="nl")
                if not trans:
    ...
    def import_record(self):
            ...
            if index_name is not None and index_name.text == "VanAbbeCollectie":
                ...
                if len(brains) == 1:
                    ...
                    if missing_lang == "nl":
                        ...
                        log_to_file(
                            f"{ccObjectID} English version of object is created"
                        )
        ...
        log_to_file(
            f"Processed range: {start_range}-{end_range} (Start: {start_time}, Finish: {finish_time})"
        )
        ...
        for record in records:
            ...
            if index_name is not None and index_name.text == "VanAbbeBibliotheek":
                ...
                # Object doesn't exist, so we create a new one
                    ...
                    if not title:
                        title = (
                            "Untitled Publication"  # default value for untitled objects
                        )
                    ...
                    try:
                        ...
                        log_to_file(
                            f"the eng translation object was not able to create"
                        )
            ...
            while retries < MAX_RETRIES and success == False:
                try:
                    if (
                        ...
                        and index_name.text == "VanAbbeBibliotheek"
                    ):
                        container = get_base_folder(self.context, "publication")
                        container_en = get_base_folder(self.context, "publication_en")
                        ...
                        index_name is not None and index_name.text == "VanAbbeCollectie"
                    ):
                        container = get_base_folder(self.context, "artwork")
                        container_en = get_base_folder(self.context, "artwork_en")
                        import_one_record(
                        ...
                        index_name is not None
                        and index_name.text == "VanabbeTentoonstellingen"
                    ):
                        container = get_base_folder(self.context, "exhibition")
                        container_en = get_base_folder(self.context, "exhibition_en")
                    ...
                    log_to_file(f"Error importing record: {record}. error = {e}")
                    transaction.abort()
    ...
    def serial_import(self):
        ...
        for offset in range(int(start_range), int(total_count), 500):
            try:
                self.sync_new_objects(
                    start_range=offset, end_range=offset + 500, date_from=date_from
                )
            except Exception as e:
